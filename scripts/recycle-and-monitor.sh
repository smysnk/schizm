...
run_prefixed "api" "kubectl -n ${NAMESPACE} logs deploy/${API_DEPLOYMENT} --follow --tail=0" &
PIDS+=("$!")
...
printf '[%-7s] %s\n' "action" "Restarting ${API_DEPLOYMENT} in ${NAMESPACE}"
...
printf '[%-7s] %s\n' "action" "Waiting for rollout status (${API_DEPLOYMENT})"
kubectl -n "$NAMESPACE" rollout status deployment "$API_DEPLOYMENT"
