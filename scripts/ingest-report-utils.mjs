...
import { captureGitHubDefaultEnvironment } from "./github-actions-env.mjs";
...
export function buildGitHubSourceContext(options = {}, env = process.env) {
  ...
  const repositoryFullName = trimToNull(event?.repository?.full_name) || trimToNull(env.GITHUB_REPOSITORY);
  const repositoryUrl =
    trimToNull(event?.repository?.html_url) || (repositoryFullName ? `${serverUrl}/${repositoryFullName}` : null);
  const branch = resolveBranch(env, event);
  const tag = resolveTag(env, event);
  const startedAt =
    normalizeTimestamp(options.buildStartedAt) || normalizeTimestamp(env.TEST_STATION_BUILD_STARTED_AT) || new Date().toISOString();
  const completedAt =
...
function resolveTag(env, event) {
  return (trimToNull(env.GITHUB_REF_TYPE) === "tag" ? trimToNull(env.GITHUB_REF_NAME) : null) || trimToNull(event?.release?.tag_name);
}
