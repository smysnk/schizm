...
import { createIngestPayload, normalizeStorageOptions, publishIngestPayload } from "./ingest-report-utils.mjs";

async function main() {
  const args = parseArgs(process.argv.slice(2));
  const reportPath = args.input || process.env.TEST_STATION_INGEST_INPUT || "./.test-results/schizm-test-report/report.json";
  ...
  process.stdout.write(
    `Published ${payload.projectKey}:${payload.source.provider}:${payload.source.runId || "manual"} to ${endpoint}\n`
  );
