...
export function CanvasGraphTab({
  ...
  const [graphMode, setGraphMode] = useState<CanvasGraphMode>("graph");
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (rawGraphTuning) {
        const parsed = JSON.parse(rawGraphTuning) as Partial<CanvasGraphTuningSettings>;
  ...
  const handleRequestLanesSnapshot = async (focusNodeId: string, focusHistory: string[]) => {
    if (!selectedCanvasPath) {
      return null;
    }

    setLaneStateByCanvas((current) => ({
      ...current,
      [selectedCanvasPath]: { focusNodeId, focusHistory }
    }));

    const result = await refetch({
      canvasPath: selectedCanvasPath,
      laneFocusNodeId: focusNodeId,
      laneFocusHistory: focusHistory,
      highlightedNotePaths
    });
    ...
    canvasFiles.length > 1 ? (
      <label className="graph-surface__selector graph-menubar__selector">
        ...
        <select
          ...
          onChange={(event) => handleCanvasPathChange(event.target.value || null)}
        >
